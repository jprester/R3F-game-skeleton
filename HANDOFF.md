...
- **Kaeldyn Tower (Operation 02):** the 41st floor of Kaeldyn Industries' tower at night, styled after the concept art in `screenshots/` (dark stone, warm linear LED strips, corporate slogans, floor-to-ceiling glass onto a generated night skyline). Insertion and extraction are in the roof-access stairwell (south-east), placed so the corridor guard cannot see it. East route: service corridor and security office, two armed KSEC guards, tall cover, and a side door into the server room that the sysadmin cannot see. West route: break room, then a large open office with desk clusters (1.2 m partitions hide a crouched player from the alarm guard but not from the two ceiling cameras above the centre line), then the server-room door the sysadmin faces. Everything is still one floor: the engine assumes a single floor plane, so a real rooftop or multi-floor level needs engine work first.
...
20. Added Kaeldyn Tower (Operation 02) with a small level kit for walls and doorways, and moved level-specific menu and briefing text into the level data. The layout was validated with probes: insertion safe from every observer, both routes connected, camera coverage leaving timeable gaps.
...
| `src/game/levels.ts` | Level kit (`shell`, `wallAlongX`, `wallAlongZ`: 3.6 m walls with 2.8 m doorways and lintels), plus the **active level source of truth:** briefing text fields, solids, precomputed bounds, play area, spawn/core/extraction, guard routes and armed flags, worker/panel, lights, signs |
...
Vite requests port **3000** and may select another port if occupied; use the URL it prints. Node **22.6+** is needed for the TypeScript-stripping test runner. `npm run build` includes TypeScript checking. At this handoff, `npm test` passes **85 tests** and `npm run build` passes. The build emits a large-chunk warning, but no error. Browser pointer lock may be unavailable inside an embedded preview; the **Use drag-look controls** fallback is intentional. No development server needs to remain running after verification.
